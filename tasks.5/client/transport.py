...
# Logger and object blocks for works with socet
logger = logging.getLogger('client')
socket_lock = threading.Lock()


# Класс-Транспорт,отвечает за сервером
...
class ClientTransport(threading.Thread, QObject):
    ...
    def connection_init(self, port, ip):
        ...
        # Функция, генерирующая приветственное сообщение для сервер
    ...
    def process_server_ans(self, message):
        ...
        # Если это сообщение от пользователя добавляем в базу, даём сигнал о новом сообщении
                ...
                and MESSAGE in message and message[DESTINATION] == self.username:
            logger.debug(f'Получено сообщение от пользователя {message[SENDER]}:{message[MESSAGE]}')
