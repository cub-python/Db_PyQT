...
class ServerStorage:
    ...
    def __init__(self, path):
        ...
        # Создаем табл контактов юзеров
        contacts = Table('Contacts', self.metadata,
                         Column('id', Integer, primary_key=True),
                         Column('user', ForeignKey('Users.id')),
                         Column("contacts", ForeignKey('Users.id'))
                         )
        # Создаем табл истории юзеров
    ...
    def user_logout(self, username):
        ...
        # Применяем изменения
        self.session.commit()
    #ФУНКЦИЯ фиксирует передачи сообщ и делает соотв отметки в БД
    ...
    #функция добавляет контакт для юзера
    def add_contact(self, user, contact):
        #получаем ID users
        user = self.session.query(self.AllUsers).filter_by(name=user).first()
        contact = self.session.query(self.AllUsers).filter_by(name=contact).first()
        # проверяем что не дубль
        if not contact or self.session.add(self.UsersContacts).filter_by(user=user.id, contact=contact.id).count():
            return
        #создаем обьект и заносим его в базу
    ...
    #Функция возврашает кол-во переданных и полученных
